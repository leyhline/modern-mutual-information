function result = shiftmiDoubleBootstrapWrapper( x, y, varargin )
%   SHIFTMIDOUBLEBOOTSTRAPWRAPPER Easier interface to calculating shifted mutual in parallel.
%   SHIFTMIDOUBLEBOOTSTRAPWRAPPER(x, y) shifts data vectors and calculates mutual
...
x = int32(x);
y = int32(y);
minmaxOfData = double([0 1 0 1]);  % This isn't considered but needs to be passed to the function below.
p = inputParser;
...
result = shiftmiDoubleBootstrap(shiftRange, binSizes, minmaxOfData, x, y, nrSamples, nrRepetitions, shiftSteps);
end
